...
# =====================================================
    ...
    calculated_cols = [
        ...
        if col not in measures + dimensions
    ]
    ...
    with colA:
        st.metric("Tables (Dataset Views)", 1)

    with colB:
        st.metric("Calculated Fields", len(calculated_cols))

    with colC:
        st.metric("Planning Columns", len([c for c in df.columns if "alloc" in c.lower() or "forecast" in c.lower()]))

    # =================================================
    # LEFT / RIGHT STORY LAYOUT
    # =================================================

    left, right = st.columns(2)

    # =================================================
    # LEFT PANEL (MODEL + TABLES)
    # =================================================
    ...
    with left:
        ...
        if view_type == "Main Table":
            st.dataframe(df, use_container_width=True)

        elif view_type == "Calculated Fields":
            if len(calculated_cols) > 0:
                st.write("### Calculated Columns")
                st.write(calculated_cols)
                st.dataframe(df[calculated_cols + measures], use_container_width=True)
            else:
                st.info("No calculated fields found")

        elif view_type == "Planning Outputs":

            planning_cols = [
                c for c in df.columns
                if any(x in c.lower() for x in ["alloc", "forecast", "copy", "version"])
            ]

            if planning_cols:
                st.dataframe(df[planning_cols + measures], use_container_width=True)
            else:
                st.info("No planning outputs found")

        elif view_type == "Forecast Data":

            forecast_cols = [
                c for c in df.columns if "forecast" in c.lower()
            ]

            if forecast_cols:
                st.dataframe(df[forecast_cols + measures], use_container_width=True)
            else:
                st.info("No forecast data found")

    # =================================================
    # RIGHT PANEL (VISUAL STORY BUILDER)
    # =================================================

    with right:

        st.markdown("## 📈 Story Charts")
        ...
        elif chart_mode == "Trend + Forecast":

            temp = df.copy()

            temp["Forecast"] = temp[y_axis] * 1.1
    ...
    # =================================================
    # FULL DATA PREVIEW SECTION
    # =================================================

    st.divider()

    st.markdown("## 📋 Full Story Preview")
    ...
    with summary_col1:
        st.write("### Measures Used")
        st.write(measures)

    with summary_col2:
        st.write("### Dimensions Used")
        st.write(dimensions)
